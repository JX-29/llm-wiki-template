...
1. Orient: read `SCHEMA.md` and **resolve the layout before any write** (paths below assume the default dialect — substitute the resolved ones), then `index.md`, and grep the wiki for the entities/concepts mentioned — don't create duplicates.
2. Save the raw source under `raw/` (articles / papers / transcripts) with frontmatter: `source_url`, `ingested`, `sha256` (over the body only). If it's a YouTube URL, use the `youtube-content` skill to fetch the transcript first, then save that.
